...
def home():
    ...
    search_criteria = {}
    projection = {'title', "duration", "difficulty",
                  "category", "image_url", "date_of_post", "author"}
    show_per_page = 4
    ...
    if request.method == "GET":
        if search_title:
            search_criteria["title"] = re.compile(
                r'{}'.format(search_title), re.I)
            cursor = client[DB_NAME].programmes.find(search_criteria, projection).skip(
                (show_per_page)*(current_page-1)).limit(show_per_page)
